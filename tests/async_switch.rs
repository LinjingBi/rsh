...
fn test_preamble_and_body_placement_async() {
    ...
    project.with_dir(|| {
        ...
        // Preamble should be before async function
        let preamble_pos = generated.find("use std::fs;").unwrap();
        let async_fn_pos = generated.find("async fn __rsh_session()").unwrap();
        assert!(preamble_pos < async_fn_pos);
        
        // Body should be inside async function
